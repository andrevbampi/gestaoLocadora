...
public class FrmConsultaUsuarios extends JDialog {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					FrmConsultaUsuarios frame = new FrmConsultaUsuarios(new Usuario(0), false);
	...
	public FrmConsultaUsuarios(Usuario usuarioSistema, boolean retornarIdParaParent) {
		...
		tblUsuarios.setModel(new DefaultTableModel(
			...
			new String[] {
				"Id", "Login", "Nome", "G\u00EAnero", "Telefone", "Administrador?"
			}
		) {
	...
	private void carregar() {
		...
		try {
			List<Usuario> listaUsuario = usuarioController.listarUsuarios(usuario, cbxVerifAdministrador.isSelected());
