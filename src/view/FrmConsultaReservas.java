...
public class FrmConsultaReservas extends JDialog {
	...
	public FrmConsultaReservas(Usuario usuarioSistema, boolean retornarIdParaParent) {
		...
		edtIdCliente.addKeyListener(new KeyAdapter() {
			@Override
			public void keyTyped(KeyEvent e) {
				ViewUtils.restringirJTextFieldApenasNuMeros(e);
			}
		...
		tblReservas.setModel(new DefaultTableModel(
			...
			new String[] {
				"Id", "Cliente", "Ve\u00EDculo", "Status", "Contrata\u00E7\u00E3o", "Pevis\u00E3o", "Devolu\u00E7\u00E3o", "Valor diária", "% Multa diária", "Multa total", "Valor prev.", "Valor total"
			}
		) {
			Class[] columnTypes = new Class[] {
				Integer.class, String.class, String.class, String.class, String.class, String.class, String.class, String.class, String.class, String.class, String.class, String.class
			};
	...
	private void carregar() {
		...
		try {
			...
			for (Reserva reservaLista : listaReservas) {
				...
				registroDb.add(ViewUtils.formatarPrecoEmReais(reservaLista.getValorDiaria()));
				registroDb.add(ViewUtils.formatarPercentual(reservaLista.getPercMultaDiaria()));
				registroDb.add(ViewUtils.formatarPrecoEmReais(reservaLista.getMultaTotal()));
				registroDb.add(ViewUtils.formatarPrecoEmReais(reservaLista.getValorTotalPrev()));
				registroDb.add(ViewUtils.formatarPrecoEmReais(reservaLista.getValorTotal()));
	...
	private void gerarRelatorio() {
		if (tblReservas.getSelectedRow() != -1) {
			try {
				int idReserva = Integer.parseInt(tblReservas.getModel().getValueAt(tblReservas.getSelectedRow(), 0).toString());
				reservaSelecionada = reservaController.buscarPorId(idReserva);
